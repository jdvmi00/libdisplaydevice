...
TEST_F(UndockRecovery, ClosedLidRetainsRecoveryUntilPanelAppears) {
  ...
  EXPECT_EQ(win_utils::flattenTopology(writes[writes.size() - 2]), (std::set<std::string> {"panel", "stream"}));
  EXPECT_EQ(clears, 1);
  writes.clear();
  EXPECT_EQ(manager->revertSettings(), SettingsManager::RevertResult::Ok);
...
TEST_F(UndockRecovery, PreservesUnrelatedActiveOutput) {
  ...
  init();
  EXPECT_EQ(manager->revertSettings(), SettingsManager::RevertResult::Ok);
  EXPECT_EQ(win_utils::flattenTopology(active), (std::set<std::string> {"panel", "other"}));
}
